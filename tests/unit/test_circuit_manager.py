...
def unsupervised_configuration():
    return {
        "model": {"type": "unsupervised", "wires": 2, "layers": 1},
        "data": {"y_range": [-1, 1]},
    }
...
@pytest.fixture
def circuit_manager(
    unsupervised_configuration, mock_unsupervised_circuit, mock_aer_data
):
    with (
        patch(
            "qcp.models.circuits.circuit_manager.load_yaml",
            return_value=unsupervised_configuration,
        ),
        patch(
            "qcp.models.circuits.circuit_manager.get_circuit",
            return_value=(mock_unsupervised_circuit, None),
        ),
    ):
...
def test_extract_shots_aer_sets_data(circuit_manager, mock_aer_data):
    with patch.object(
        circuit_manager, "_load_from_db", return_value=mock_aer_data["aer_test_0"]
    ):
...
def test_extract_shots_data_counts_preserved(circuit_manager, mock_aer_data):
    ...
    assert sum(circuit_manager.data.values()) == sum(
        mock_aer_data["aer_test_0"].values()
    )
